void DeleteInstance(FullSol* InitialFullSolution, CommonList* CurCommonListNew, int Pol, int PosOfChangingNF, float BWChanged, float NoOfServers, float NoOfLinks, short int MaxLinksCap, int w1, int w2, int w3, int w4, int w5, ChangedPolicies* TrackChangedPolicies){
	...
	short int NewBW = OldBW - BWChanged;
			...
			if ( NFtoChangeNewSvr != NFtoChangeCurSvr){
				...
				if(PosOfChangingNF == (TotNFs-1) ){ // last NF in the chain
					...
					if( NoOutPath == false){ // there is an out path, so continue
						...
						Update_LinksMatrix(InitialFullSolution,OutPathNew, NewBW,"D");
						...
						if(NFtoChangeNewSvr != LeftOfNFtoChangeSvr){
							ReturnPath RPath;
							RPath = Find_Shortest_PathRandom(LeftOfNFtoChangeSvr, NFtoChangeNewSvr, CurCommonListNew, &InitialFullSolution->CurLists, NewBW);
					...
					if( NoInPath == false){ // there is an InPath
						...
						if(NFtoChangeNewSvr != RightOfNFtoChangeSvr){
							ReturnPath RPathNew;
							RPathNew = Find_Shortest_PathRandom(NFtoChangeNewSvr,RightOfNFtoChangeSvr, CurCommonListNew, &InitialFullSolution->CurLists, NewBW);
