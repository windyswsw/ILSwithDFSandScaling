void ScaleAcceptanceCriteria(FullSol* FullSolution, CommonList* CurCommonLists, float NoOfServers, float NoOfLinks, short int MaxLinksCap, ChangedPolicies* TrackChangedPolicies){
	...
	if(InitialObj < CurObj){ // Initial Solution is better
		...
		// since current solution is not better, remove all CurrentlyChangedPolicies
		TrackChangedPolicies->CurrentlyChangedPolicies.clear();

		FullSolution->ObjVal = InitialObj;
		float NewObj = LocalReturnObjectValue(FullSolution, CurCommonLists, NoOfServers, NoOfLinks, MaxLinksCap, 1, 1, 1, 1, 1, TrackChangedPolicies);

	}
	if(InitialObj >= CurObj){ // current solution is better, so move all CurrentlyChangedPolicies to FirstLocalSearchPolicies, so that they are permanantly saved
