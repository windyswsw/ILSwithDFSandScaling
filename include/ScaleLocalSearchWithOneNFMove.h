bool ScaleLocalSearchWithOneNFMove(FullSol* InitialFullSolution, CommonList* CurCommonListNew, int policyIndex, int PosOfChangingNF, float NoOfServers, float NoOfLinks, short int MaxLinksCap, PartialSol* SelectedPolicyOriginal, int w1, int w2, int w3, int w4, int w5, RoundChangeInfo* RoundChangeNow, ChangedPolicies* TrackChangedPolicies){
	...
	SelectedPolicyTemp.Splits = SelectedPolicyOriginal->Splits;
	SelectedPolicyTemp.VMsForAPolicy = SelectedPolicyOriginal->VMsForAPolicy;
								...
								if(NoNFPath == false){ // there is a path so continue
									...
									if(NoNewLinks == false){ // there are 2 or more new links in the path
										...
										if(NFPath.Path.size() > 1 ){
											Update_LinksMatrix(InitialFullSolution, NFPath, BW,"D"); // add the new path from left NF by deleting BW from relavant links
										}
										...
										SelectedPolicyTemp.VMsForAPolicy[PosOfChangingNF] = NewNF;

										TempChangesCount Count  = CheckNumberOfChangesTemp(TrackChangedPolicies, CurCommonListNew, &SelectedPolicyTemp, policyIndex);
										float NewObj = LocalTempObjectValue(InitialFullSolution, CurCommonListNew, NoOfServers, NoOfLinks, MaxLinksCap, w1, w2, w3, w4, w5, Count.Svr, Count.Link);
											
										if(OldObjVal > NewObj){ // obj is better, so save all changes in the original policy
										...
										if(OldObjVal <= NewObj) {
											// obj is not better, so revert back the in path from left NF
					...
					if( NoInPath == false){ // there is an InPath
						...
						if(NoNewLinks == false){
							...
							if( NoPathForRightNF == false){ // there is a path to right NF or they are in same server
								...
								SelectedPolicyTemp.VMsForAPolicy[PosOfChangingNF] = NewNF;
								SelectedPolicyTemp.VMsForAPolicy[PosOfChangingNF+1] = NewNFforRightNF;
								
								TempChangesCount Count  = CheckNumberOfChangesTemp(TrackChangedPolicies, CurCommonListNew, &SelectedPolicyTemp,policyIndex );
								float NewObj = LocalTempObjectValue(InitialFullSolution, CurCommonListNew, NoOfServers, NoOfLinks, MaxLinksCap, w1, w2, w3, w4, w5, Count.Svr, Count.Link);
					
								if(OldObjVal > NewObj){ // obj is better, so proceed
								...
								if(OldObjVal <= NewObj) { // old solution is better, revert back

									Update_LinksMatrix(InitialFullSolution,  InPath, BW,"A"); // remove the newly assigned In path
									Update_LinksMatrix(InitialFullSolution,  RightOfNFPath, BW,"A"); // remove the newly assigned right path
							...
							if(NoNFPath == false){ // there is a path so continue
								...
								if(NoNewLinks == false){ // there are 2 or more new links in the path
									...
									if( NoPathForRightNF == false){ // there is a path to right NF or they are in same server
										...
										if(OldObjVal <= NewObj) { 
											// obj is not better, so revert back the in path and right path
											Update_LinksMatrix(InitialFullSolution, NFPath1, BW,"A");
											Update_LinksMatrix(InitialFullSolution,  RightOfNFPath, BW,"A");
